...
public final class DraftAiExporter {
    public String export(DraftPickState state) {
        ...
        for (long id : state.offeredCardIds()) appendCard(out, id, 1, state.cards());
        out.append("\n[CURRENT COLLECTION]\n");
        for (DraftCardCount entry : state.draftedPool()) {
            appendCard(out, entry.arenaId(), entry.quantity(), state.cards());
        }
        out.append("\n[CURRENT MAIN DECK]\n");
        appendCounts(out, state.mainDeck(), state.cards());
        out.append("\n[CURRENT SIDEBOARD]\n");
        appendCounts(out, state.sideboard(), state.cards());
    ...
    private void appendCounts(StringBuilder out, List<DraftCardCount> counts, Map<Long, CardInfo> cards) {
        if (counts.isEmpty()) out.append("(not observed)\n");
        for (DraftCardCount entry : counts) appendCard(out, entry.arenaId(), entry.quantity(), cards);
    }

    private void appendCard(StringBuilder out, long arenaId, int quantity, Map<Long, CardInfo> cards) {
