...
2. **Shared replay-style candidate chips.** Extract or expose the card-chip presentation used by the replay view as a reusable component/painter and use that same renderer for resolved consideration entries. Candidate rows show card identity/presentation and stale state, but no owned-count text while collection ownership is deferred.
   ...
   Implementation status (2026-08-07): implemented in the pending step-2 patch by extracting the replay hover-preview wrapper into reusable `app.replay.ReplayCardChip`, keeping `GameView` on that same component, and using it as the renderer for resolved Under consideration rows. Stale entries remain explicit recoverable text rows. Ownership text is omitted even when a quantity source is supplied. Human-side Maven validation is required before step 3.
...
`DP-06 — Under consideration workspace` is active in its human-feedback rework phase. Rework step 1 is validated at a 221-test green baseline; step 2 (shared replay-style candidate chips with ownership presentation omitted) is the current implementation slice. Follow the six-step DP-06 rework plan above; do not start DP-07 until the real-card click harness is explicitly accepted.

### Current planning decisions

- DP-01, DP-03, DP-04, and DP-05 are complete. DP-02 contracts are implemented; live ownership integration is deferred because the current client does not publish authoritative collection quantities in `Player.log`. DP-06 rework step 1 has a clean 221-test baseline at commit `bcf61885455c5858a94876bf0f3b9e25b225b0bd`; human review on 2026-08-07 reopened its acceptance surface for the six-step real-card/candidate-interaction rework above. Ownership-dependent consideration displays are omitted while `SA-MTGA-DEF-003` remains open.
