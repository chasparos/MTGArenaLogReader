...
- **Alternate-art / favorite-printing / legal-state slice (2026-08-08).** Resolve candidate identities against the active catalog first and the persistent card cache second so known off-format cards remain visible as `Illegal` rather than collapsing into stale/unavailable state. Enumerate alternate printings through the shared name/Scryfall path, persist one favorite Scryfall printing per logical identity, and expose a click picker from Candidate chips. Keep favorite art independent from legality: a favorite printing does not make an illegal logical card legal.
